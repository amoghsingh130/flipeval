...
def nlines(text: str, width_cm: float) -> int:
    ...
    return max(1, int(math.ceil(chars / per_line)))
...
def emit_tikz(data: dict) -> str:
    ...
    for entry in rows:
        lines = max(nlines(entry["label"], lab_w),
    ...
    a(rf"\node[ttl, text width={a_w}cm, inner sep=0] at ({a_x},{y:.2f}) "
      r"{A\quad As reported today};")
    a(rf"\node[ttl, text width={b_w}cm, inner sep=0] at ({b_x},{y:.2f}) "
      r"{B\quad The same evaluation, reported to the standard};")
    ...
    for entry in rows:
        a(rf"\node[lbl, text width={lab_w}cm, inner sep=0] "
          ...
          rf"{{\textbf{{{entry['n']}}}\quad {entry['label']}}};")
        ...
        if entry["n"] == 2:
            ...
            a(rf"\node[lbl, text width={b_w}cm, inner sep=0] "
              rf"at ({b_x},{y - 0.04 - 1.30:.2f}) {{{entry['b']}}};")
        else:
            a(rf"\node[lbl, text width={b_w}cm, inner sep=0] "
