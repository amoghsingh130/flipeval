...
are ratios of medians, not medians of cellwise ratios: the
median of the per-cell ratio is 3.85, and the two summarise different things.
That figure is taken over the 1{,}562 cells whose net delta is non-zero, because
a per-cell ratio is undefined for the other 145
(\S\ref{sec:atlas:identical}). Dropping them is the conservative choice rather
than a convenient one: 128 of the 145 have non-zero churn, which is the most
complete cancellation in the atlas, so any convention that keeps them sorts them
above every finite ratio and raises the median, to 4.20 if they are admitted as
unbounded. The pooled figure describes the typical churn against the typical net
delta; the cellwise median describes the typical cell. Neither is the headline on
its own.
