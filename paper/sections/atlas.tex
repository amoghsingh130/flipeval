...
%   POOLED, at full precision: 0.120000 / 0.0222222 = 5.4000 exactly.
...
%
% PRECISION CONVENTION, stated because the three figures are NOT computed
% identically and that must not be hidden. The pooled 5.40 divides unrounded
% medians. The stratum 5.27/5.33 divide the medians AT THE PRECISION THE TABLE
% PRINTS (3 dp), so a reader can reproduce them from the printed table; divided
% unrounded the strata give 5.2232 and 5.1936. The prose says which is which.
% Do not "harmonise" these into one number computed two ways.
%
...
accuracy change is \textbf{5.40} pooled over the 1{,}707 cells, and 5.27 and
5.33 within S1 and S2, the stratum figures computed from the medians at the
precision Table~\ref{tab:atlas-strata} prints so that a reader can reproduce
them from it. These are ratios of medians, not medians of cellwise ratios: the
median of the per-cell ratio is 3.85 over the 1{,}562 cells with a non-zero net
delta, and the two summarise different things. The pooled figure describes the
typical churn against the typical net delta; the cellwise median describes the
typical cell. Neither is the headline on its own.
