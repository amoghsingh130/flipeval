...
% headline is a ratio of medians. The like-for-like atlas number is the median
% of its per-cell ratios, 3.85 over the 1,562 cells with a non-zero net delta.
% Both are now given, each against its own counterpart, and the comparison the
% Result actually makes is the like-for-like one. It is the stronger of the two.
...
quantized-against-FP16 contrast. Comparing like with like, the median of the
per-cell ratios is \textbf{3.85} across the atlas
(\S\ref{sec:atlas:netgross}) and \textbf{12.7} across the eight controlled
cells, where the per-cell ratios run \textbf{$3.0\times$ to $30.5\times$}. On
the other aggregation, the ratio of medians, the atlas gives 5.40.
\end{result}
...
% 3.04 / 12.71 / 30.45 are arithmetic on panel (c) of tab:h3-supporting.
