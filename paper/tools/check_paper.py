...
REVIEW_ONLY_PATHS = [
    ...
    ("paper/OUTLINE.md",
     "dated planning record, never compiled into the PDF and not \\input by "
     "main.tex. It records what the paper was PLANNED to argue as of its "
     "Status line, including counts later withdrawn, and editing those to the "
     "current values would destroy the only record of what changed. Same "
     "reason docs/ is review-only, and hit_context already classifies this "
     "file as planning-not-compiled. NOT an allowlist entry: the hits stay "
     "visible, so a number that migrates from here into a compiled section is "
     "still caught at its destination"),
]
